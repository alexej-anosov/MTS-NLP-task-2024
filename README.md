...
## Введение
Целью данного проекта является создание агента на основе небольшой (до 7B params) open-source языковой модели, обратившись к которому пользователь может заказать себе билеты на самолёт. В качестве модели была выбрана Mistral7B-Instruct-v0.3. В качестве языка общения используется английский язык.

На данный проект использует синтетические данные и mock функции, однако в будущем возможно подключение настоящих API.

## Ход работы
### Данные
Использованы сгенерированные самостоятельно данные о перелётах за Июль 2024 года между 87 городами. Перелеты между двумя городами отличаются числом пересадок, временем отправки и прибытия, временем в воздухе, классом билета и ценой. Это позволяет иммитировать различные сценарии общения пользователя с моделью. Подробнее о генерации данных можно прочитать в [`report`](report.pdf).
...
| get flights  | Позволяет получить возможные рейсы. В качестве параметров передаются критерии фильтрации датасетов, а также способ сортировки (подробнее возможные параметры можно посмотреть в [файле](https://github.com/alexej-anosov/MTS-NLP-task-2024/blob/c90a166a705bc9f7cd006a0be3930e05b4595c82/src/tools.py#L221)) |
...
Исходная модель была дообучена на данных примерах с помощью LoRA. Результаты можно посмотреть [здесь](https://wandb.ai/alexej-anosov/MTS-NLP-task-2024). 

## Результаты
В результаты дообучения модели ее поведение значительно улучшилось. Если для базовой модели получить правильный ответ было практически невозможно, с дообученной это чаще всего можно сделать. Однако, модель регулярно совершает лишние действия и иногда путается. В связи с этим из-за штрафов за лишние действия скор модели все еще существенно ниже, чем у Gpt-4o. Однако, по мере увеличения числа данных в обучающей выборке, качество может быть сильно улучшено с использованием того же пайплайна обучения.
...
* Автоматизировать оценивание работы модели с помощью API более крупной модели. Так как критерии оценки четко сформулированы, такое решение должно работать хорошо. Для более стабильной оценки можно усреднять по нескольким прогонам оцениваемой и оценивающей моделей.
* Автоматизировать сбор тренировочного датасета с помощью API более крупной модели. Возможно, даже без отбора неудачных сэмплов можно получить существенное улучшение качества.

Это даст возможность прооводить много экспериментов, например со структурой промпта, фреймворком поведения агента (Tree-of-Thoughts, Reflexion, ExpeL), различными моделями в качестве основы.
...
1) Inference. Пользователь вводит запрос вручную. Общение с агентом происходит через командную строку. Для запуска модели необходимо выполнить команду:
    ...
    ```
    Доступные конфиги находятся в папке [./configs/inference](configs/inference).
    
2) Evaluation. Агент сам итерируется по тестовому датасету. Общение с агентом происходит через командную строку. После каждого сэмпла (диалога) пользователь должен оценить качество ответа модели (согласно методологии из [`report`](report.pdf)). Для запуска модели необходимо выполнить команду:
    ```
    python main.py --config_path <config_path>
    ```
    Доступные конфиги находятся в папке [./configs/evaluation](configs/evaluation).
    
3) Dataset collection. Пользователь вводит запрос вручную. Общение с агентом происходит через командную строку. После каждого действия модели пользователь должен оценить его (1 или 0). В случае положительной оценки пример добавляется в датасет. Для запуска модели необходимо выполнить команду:
    ```
    python main.py --config_path <config_path>
    ```
    Доступные конфиги находятся в папке [./configs/dataset_collection](configs/dataset_collection)


## Дообучение модели
Код для дообучения модели находится в папке [./finetuning](finetuning).
